...
#define serie 10 //Es una constante
...
int main() {
  ...
  int vector[serie] = {1,2,5,71,99,0,5,16,26,3};

  SelectionSort(vector,serie);
  Print("SelectionSort",vector,serie);

  Shell(vector,serie);
  Print("Shell",vector,serie);

  IBinaria(vector,serie);
  Print("Isercion Binaria",vector,serie);

  IDirecta(vector,serie);
  Print("Isercion Directa",vector,serie);

  QuickSort(vector,0,serie - 1); // serie - 1: resto 1 porque es la posición inicial menos la final POSICIÓN NO CANTIDAD DE ELEMENTOS
  Print("QuickSort",vector,serie);

  Burbuja(vector,serie);
  Print("Burbuja",vector,serie);
